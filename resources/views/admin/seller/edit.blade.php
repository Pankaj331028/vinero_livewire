...
<x-breadcrumb title="Edit seller" module="seller" link="{{url($previous_url)}}"></x-breadcrumb>
<!-- begin:: Content -->
                        ...
                        <div class="card-body">
                            <div class="kt-portlet">
                                ...
                                <form class="kt-form kt-form--label-right form-valide"
                                    action="{{ route('update-seller', $seller->id) }}" method="post"
                                    enctype="multipart/form-data" id="seller_form">
                                    {{csrf_field()}}
                                        ...
                                        <div class="row">
                                            <div @if($status == 1) class="form-group col-lg-6" @else class="form-group col-lg-4" @endif>
                                                ...
                                                <textarea class="form-control" id="comment_note"
                                                    name="comment_note">{{old('comment_note', $seller->comment_note)}} </textarea>
                                            </div>
                                            <div @if($status == 1) class="form-group col-lg-6" @else class="form-group col-lg-4" @endif>
                                            ...
                                            <div class="form-group col-lg-4">
                                                ...
                                                <select class="form-control" name="optin_out" id="optin_out">
                                                    <option value="">Select status</option>
                                                    <option value="OPTOUT" {{ $seller->optin_out == 'OPTOUT' ? 'selected' : '' }}>OPTOUT</option>
                                                    <option value="OPTIN" {{ $seller->optin_out == 'OPTIN' ? 'selected' : '' }}>OPTIN</option>
                                                </select>
                                            </div>
                                            @else
                                            @endif

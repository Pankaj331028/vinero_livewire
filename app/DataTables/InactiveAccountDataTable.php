...
class InactiveAccountDataTable extends DataTable {
	/**
	...
	public function dataTable($query, Request $request) {
		...
		$query = Admin::select('model_has_permissions.*', 'name', 'id')->where('role', Role::whereName('subadmin')->first()->id)->join('model_has_permissions', 'model_id', '=', 'admin.id')->whereStatus('IN');
		...
		$query->groupBy('model_has_permissions.model_id')->groupBy('model_has_permissions.module');
		...
		return datatables()
			...
			->addcolumn('action', function ($row) {
				...
				return $btn;
			})->rawColumns(['created_at', 'last_activity', 'no_audits', 'link_name', 'action']);
	}
	...
	public function html() {
		return $this->builder()
			->setTableId('inactiveDataTable')
			->columns($this->getColumns())
			->minifiedAjax('', "data.table='inactiveDataTable'")
			->parameters([
				'rowGroup' => [
					'dataSrc' => ['link_name'],
				],
	...
	protected function filename() {
		return 'BenchManager_' . date('YmdHis');
	}
}

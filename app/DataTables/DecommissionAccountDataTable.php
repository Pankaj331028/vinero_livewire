...
class DecommissionAccountDataTable extends DataTable {
	/**
	...
	public function dataTable($query, Request $request) {
		...
		$query = Admin::where('role', Role::whereName('subadmin')->first()->id)->doesntHave('permissions');
		...
		return datatables()
			...
			->addcolumn('action', function ($row) {
				...
				return $btn;
			})->rawColumns(['created_at', 'module', 'control', 'last_activity', 'no_audits', 'link_name', 'action']);
	}
	...
	public function html() {
		return $this->builder()
			->setTableId('decomDataTable')
			->columns($this->getColumns())
			->minifiedAjax('', "data.table='decomDataTable'")
			->parameters([
				'rowGroup' => [
					'dataSrc' => ['link_name'],
				],
	...
	protected function getColumns() {
		return [
			...
			Column::make('id')->data('DT_RowIndex')->name('S.No.'),
			// Column::make('module')->title('Account'),
	...
	protected function filename() {
		return 'DecomissionManager_' . date('YmdHis');
	}
}

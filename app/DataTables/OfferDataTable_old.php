...
class OfferDataTable_old extends DataTable {
	use Helper;
	/**
	...
	public function dataTable($query) {
		if ($this->type == 'agent') {
			$query = Offers::where('updated_by', '=', $this->user_id)->where('status', '!=', 'IN');
		} else {
		...
		return datatables()
			...
			->addcolumn('offer_price', function ($row) {
				return $row->transaction->offer_price ?? 0.00;
			})
			...
			->addcolumn('action', function ($row) {
                ...
                <a href="' . route('offer-details', $row->id) . '" class="btn btn-sm btn-clean btn-icon btn-icon-md" title="title"><i class="la la-eye"></i></a>
                </span>';
				} else {
					$btn .= '-';
				}
